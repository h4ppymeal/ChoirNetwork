...
DEFAULT_EVAL_PATH = Path("eval/queries.jsonl")
...
def prepare_eval_queries(index: HymnIndex, path: Path) -> list[EvalQuery]:
...
def evaluate_engine(
    ...
    *,
    ...
    for item in eval_queries:
        ...
        retrieved = [match.slug for match in matches]
...
def compare_retrieval_configs(
    ...
    *,
    ...
    eval_queries = prepare_eval_queries(index, eval_path)
    if not eval_queries:
        raise SystemExit(
            f"No eval queries matched the index at {index_path}. "
            "Build the index first, then verify eval/queries.jsonl labels."
        )

    configs = [
        ("bi_encoder_only", False, False),
        ("chunked_rerank", True, False),
        ("chunked_rerank_expand", True, True),
    ]

    results: list[EvalRunResult] = []
    for name, use_reranker, expand_flag in configs:
        engine = load_engine(index_path, use_reranker=use_reranker)
